...
import { BookOpen, Bot, ChevronLeft, ChevronRight, ExternalLink, Info, ListChecks, MessagesSquare, Search } from 'lucide-react'
import {
  getMyStudyStatus,
  listCards,
  listDecks,
...
type View = 'menu' | 'intro' | 'flash' | 'qna' | 'search' | 'cbt' | 'rag'
  ...
  if (view === 'menu')
    return (
      <div className="space-y-3">
        ...
        <StudyMenuTile icon={<ListChecks size={26} />} title="CBT 문제풀이" desc="예상문제 풀고 채점·해설" onClick={() => setView('cbt')} />
        <StudyMenuTile icon={<Bot size={26} />} title="AI 상담 (질문답변)" desc="학습자료 근거로 질문답변" onClick={() => setView('rag')} />
